...
namespace Classified.Web.Controllers
{
    ...
    {
        private Services.User.ApplicationSignInManager _signInManager;
        private Services.User.ApplicationUserManager _userManager;

        public AccountController()
        {
        }

        public AccountController(Services.User.ApplicationUserManager userManager, Services.User.ApplicationSignInManager signInManager)
        {
            UserManager = userManager;
            SignInManager = signInManager;
        }

        public Services.User.ApplicationSignInManager SignInManager
        {
            get
            {
                return _signInManager ?? HttpContext.GetOwinContext().Get<Services.User.ApplicationSignInManager>();
            }
            private set
            {
        ...
        public Services.User.ApplicationUserManager UserManager
        {
            get
            {
                return _userManager ?? HttpContext.GetOwinContext().GetUserManager<Services.User.ApplicationUserManager>();
            }
            private set
            {
                _userManager = value;
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //Fetch the informtaion of the user
            ...
            if (!await UserManager.IsEmailConfirmedAsync(tempUser.Id))
            {
                ModelState.AddModelError("", "You need to confirm your email address.");
                return View(model);
            }

            if (await UserManager.IsLockedOutAsync(tempUser.Id))
            {
                return View("Lockout");
            }
        ...
        public async Task<ActionResult> Register(UserRegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //Add the user to the list of default system users
                    ...
                    await UserManager.SendEmailAsync(user.Id, "Confirm and Active your account", Services.User.EmailService.EmailConfirmationTemplate(model.FirstName,model.LastName,callbackUrl));

                    //Add Message for the user to show him/her that a confirmation email has been sent to their inbox.
                    //Console.WriteLine(e);
                    //throw;
                    ...
                    PopUpMessageGenerator.GenerateMessage("User Registration System",
                        "A confirmation email has been sent to your email address. Please check your Spam if you does not find the email in your Inbox",
        ...
        /// <summary>
        /// Submit Ads By User View Caller
        /// </summary>
        /// <returns></returns>
        [Authorize]
        public ActionResult SubmitAdsByUser()
        {
            return View();
        }
